...
function InitialPathNavigator() {
  const navigate = useNavigate();
  const initialPath = webviewConfig?.initialPath;

  useEffect(() => {
    if (!initialPath) {
      return;
    }
    void navigate({ to: initialPath as never, replace: true });
